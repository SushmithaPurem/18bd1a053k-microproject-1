...
app.get('/ventilatordetails',middleware.checkToken,(req,res)=>{
    ...
    var ventilatordetails=db.collection('ventilatorInfo').find().toArray().then(result=>res.json(result));
});

app.post('/searchventbystatus',middleware.checkToken,(req,res)=>{
    var status=req.body.status;//by status searching ventilators
    console.log(status);
    var ventilatordetails=db.collection('ventilatorInfo').find({"status":status}).toArray().then(result=>res.json(result));
});

app.post('/searchventbyname',middleware.checkToken,(req,res)=>{
    var name=req.body.name;//by hospital name searching ventilators
    console.log(name);
    var ventilatordetails=db.collection('ventilatorInfo').find({'name':new RegExp(name,'i')}).toArray().then(result=>res.json(result));
});
app.post('/searchventbyId',middleware.checkToken,(req,res)=>{
    var hid=req.body.hid;//by hospitalId  searching ventilators
    console.log(hid);
    var ventilatordetails=db.collection('ventilatorInfo').find({'hid':new RegExp(hid,'i')}).toArray().then(result=>res.json(result));
});
...
app.post('/searchhospital',middleware.checkToken,(req,res)=>{
    ...
    console.log(name);
    var hospitaldetails=db.collection('hospitalInfo').find({'name':new RegExp(name,'i')}).toArray().then(result=>res.json(result));
});
...
app.put('/updateventilator',middleware.checkToken,(req,res)=>{
    ...
    db.collection("ventilatorInfo").updateOne(ventid,newvalues,function(err,result){
...
app.delete('/delete',middleware.checkToken,(req,res)=>{
    ...
    db.collection('ventilatorInfo').deleteOne(myquery1,function(err,obj){
        if (err) throw err;
